...
module.exports = {
    ...
    execute: async (client, message, args, embed, author, channel, guild) => {
        ...
        let kav = db.get(`kke_${member.id}`);
        if (!kav) return message.reply({ embeds: [embed.setDescription("Bu kullanıcının veri tabanında kayıt verisi bulunmamakta!")] }).catch((err) => console.log(err)).then((e) => setTimeout(() => { e.delete(); }, 10000));
