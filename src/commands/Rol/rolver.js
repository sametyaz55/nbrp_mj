...
module.exports = {
    ...
    execute: async (client, message, args, embed, author, channel, guild) => {
        ...
        if (!role) return message.reply({ embeds: [embed.setDescription("Geçerli bir rol belirtmelisin.")] });
          ...
          return  message.reply({ embeds: [embed.setDescription("Verilecek rol sizin rolünüzün üstünde bu yüzden rolü **Veremiyorum!**")] });
          }
        ...
        .setDescription(`${member} isimli üyeye ${role} isimli yetki başarıyla Verildi!`)
        .setColor('#D2EE07')
        ...
        .setColor('RANDOM')
        .setDescription(`**Rol Verildi !**\n**Veren Yetkili:** ${message.author} (\`${message.author.id}\`)\n**Verilen Üye:** ${member.user} (\`${member.user.id}\`)\n**Verilen Rol:** ${role}`);
